    ...
    public class MeetingListController : ApiController
    {
        ...
        public List<MeetingList> GetMeetingByID(long UserID)
        {
            ...
            using (MySqlConnection connection = new MySqlConnection(MyConnnectionString))
            {
                ...
                { 
                    cmd.CommandText = "SELECT * FROM ui_meetinglist where  Status <> 'Done' and UserID =" + UserID;
                ...
                using (MySqlDataReader reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        meetingDetails.Add(new MeetingList
                        {
                            ...
                            SFDCLead_ID = reader["SFDC_ID"].ToString()
           ...
           using(MySqlConnection connection = new MySqlConnection(MyConnnectionString))
            {
                ...
                using(MySqlCommand cmd = connection.CreateCommand())
               { 
                cmd.CommandText = "SELECT * FROM ui_meetinglist where CustomerName ='" + name + "'";
        ...
        {   
            ...
            using (MySqlConnection connection = new MySqlConnection(MyConnnectionString))
            {
                ...
                using (MySqlCommand cmd = connection.CreateCommand())
                { cmd.CommandText = "SELECT max(ID) FROM ui_meetinglist";
                    ID = long.Parse(cmd.ExecuteScalar().ToString()) + 1;
                    cmd.CommandText = "INSERT INTO ui_meetinglist VALUES(@ID,@UserID,@LeadID, @SFDC_ID, @Subject,@StartDate,@EndDate,@CustomerName,@City,@State,@Status,@Comments)";
                    ...
                    cmd.Parameters.AddWithValue("@SFDC_ID", value.SFDCLead_ID);
